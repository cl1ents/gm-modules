...
export default {
    goosemodHandlers: {
        ...
        onRemove: async () => {
            // I DON'T KNOW HOW TO REVERT IT LOL!!
            /* const nodes = Object.values(settingsStore._dispatcher._actionHandlers._dependencyGraph.nodes);
            try {
                nodes.find(x => x.name == "ExperimentStore").actionHandler["CONNECTION_OPEN"]({user: {flags: 0}, type: "CONNECTION_OPEN"})
            } catch (e) {} // this will always intentionally throw
            const oldGetUser = userStore.__proto__.getCurrentUser;
            userStore.__proto__.getCurrentUser = () => ({hasFlag: () => false})
            nodes.find(x => x.name == "DeveloperExperimentStore").actionHandler["CONNECTION_OPEN"]()
            userStore.__proto__.getCurrentUser = oldGetUser */
            showToast("REFRESH!")
        }
